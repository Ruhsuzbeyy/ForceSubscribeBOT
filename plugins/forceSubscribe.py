    ...
    if chat_member.restricted_by:
      if chat_member.restricted_by.id == (client.get_me()).id:
          try:
            ...
            client.answer_callback_query(cb.id, text="❗🤗 Bahsedilen 'Gruba' katılın ve tekrar 'KATILDIM' düğmesine basın.", show_alert=True)
      else:
      ...
      try:
        ...
        try:
          sent_message = message.reply_text(
              " {} ,❗🤗 Henüz AnaGruba katılmadınız . Lütfen aşağıdaki düğmeyi kullanarak katılın ve Buraya yazmak  için KATILDIM düğmesine basın❗ ".format(message.from_user.mention, channel, channel),
              disable_web_page_preview=True,
             reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton("✔KATIL👈🏽", url=channel_url)
                ],
                [
                    InlineKeyboardButton("KATILDIM✅", callback_data="onUnMuteRequest")
                ]
            ]
        )
          )
